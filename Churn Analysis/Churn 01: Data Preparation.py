...
# MAGIC %md The purpose of this notebook is to access & prepare the data required for churn prediction.  It should be run on a cluster leveraging **Databricks ML 7.1+** and **CPU-based** nodes.
...
# MAGIC 
# MAGIC **NOTE** Due to the terms and conditions by which these data are made available, anyone interested in recreating this work will need to download the files from Kaggle that make up this dataset and create a similar folder structure as described below in their environment.
# MAGIC 
# MAGIC The primary data files available for download are organized under as follows under a pre-defined [mount point](https://docs.databricks.com/data/databricks-file-system.html#mount-object-storage-to-dbfs):
# MAGIC 
