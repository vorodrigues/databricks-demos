...
# MAGIC %pip install databricks-feature-engineering>=0.13.0a3 databricks-sdk>=0.62.0
# MAGIC dbutils.library.restartPython()

# COMMAND ----------

# MAGIC %run "/Workspace/Users/victor.rodrigues@databricks.com/databricks-demos/Feature Store/_resources/00-init-expert-full"
...
spark.sql(f"ALTER TABLE {catalog}.{db}.expert_predictions_labels SET TBLPROPERTIES (delta.enableChangeDataFeed = true)")
...
#     name="vr-online-store",
#     capacity="CU_"  # Valid options: "CU_1", "CU_2", "CU_4", "CU_8"
# )
...
for table in ['user_features', 'destination_features', 'destination_location_features', 'availability_features']:
    spark.sql(f"ALTER TABLE {catalog}.{db}.{table} SET TBLPROPERTIES (delta.enableChangeDataFeed = true)")
    fe.publish_table(
        online_store=online_store,
        source_table_name=f"{catalog}.{db}.{table}",
        online_table_name=f"{catalog}.{db}.{table}_online"
    )
...
wc = WorkspaceClient()
served_models =[ServedModelInput(model_name=model_full_name, model_version=latest_model.version, workload_size="Small", scale_to_zero_enabled=True)]
try:
