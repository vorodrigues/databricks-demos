...
# MAGIC   - Performance
# MAGIC   - Sumarização
# MAGIC   - Agrupamento das entidades
# MAGIC   - Fine tuning
...
# MAGIC
# MAGIC Neste notebook demonstramos a extração, processamento e análise de eventos adversos de medicamentos a partir de uma coleção de 20.000 textos do mundo real usando processamento de linguagem natural (NLP) com modelos de Inteligência Artificial Generativa (GenAI).
# MAGIC
...
# MAGIC %sql SELECT * FROM clinical_notes LIMIT 100
...
# MAGIC %sql select is_ADE, count(*) as from clinical_notes group by is_ADE
...
# MAGIC %sql SELECT AI_QUERY('databricks-dbrx-instruct',
# MAGIC   'Traduza para Português o texto a seguir (não adicione nenhum texto além da tradução):
# MAGIC    CONCLUSION: The administration of tissue plasminogen activator was responsible for the large extent of hemorrhage and should be considered in the differential diagnosis of hemorrhagic choroidal detachment.') AS text
...
def translate_to_pt(text):
  chat = ChatSession(model="databricks-dbrx-instruct", max_tokens=800)
  return chat.reply(f"Traduza para Português o texto a seguir (não adicione nenhum texto além da tradução): {text}").message
...
print(translate_to_pt('CONCLUSION: The administration of tissue plasminogen activator was responsible for the large extent of hemorrhage and should be considered in the differential diagnosis of hemorrhagic choroidal detachment.'))
...
@pandas_udf("text string")
def translate_udf(df):
  return df.apply(translate_to_pt)

# COMMAND ----------

from pyspark.sql.functions import col

display(spark.table('clinical_notes').limit(10).withColumn('text', translate_udf(col('text'))))

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE clinical_notes_pt AS
# MAGIC SELECT id, translate_to_pt(text) AS text, is_ADE FROM clinical_notes
...
# MAGIC %sql
# MAGIC SELECT * FROM clinical_notes_pt LIMIT 100
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE FUNCTION analyze_clinical_notes(note STRING)
# MAGIC     RETURNS STRUCT<presenca_evento: STRING, eventos: ARRAY<STRING>, drogas: ARRAY<STRING>>
# MAGIC     RETURN FROM_JSON(
# MAGIC         AI_QUERY(
# MAGIC             'databricks-meta-llama-3-70b-instruct',
# MAGIC             CONCAT(
# MAGIC                 'Com base na nota clínica a seguir, extraia as seguintes informações:
# MAGIC                 - identifique a presença de evento adverso: S ou N
# MAGIC                 - tipos de eventos adversos
# MAGIC                 - drogas mencionadas
# MAGIC                 Retorne somente um JSON (nenhum texto fora do JSON) com o seguinte formato:
# MAGIC                 {
# MAGIC                     "presenca_evento": "<S ou N>",
# MAGIC                     "eventos": <lista eventos>,
# MAGIC                     "drogas": <lista drogas>
# MAGIC                 } 
# MAGIC                 Nota clíncia: ', note
# MAGIC             )
# MAGIC         ),
# MAGIC         "STRUCT<presenca_evento: STRING, eventos: ARRAY<STRING>, drogas: ARRAY<STRING>>"
# MAGIC     );
...
# MAGIC %sql
# MAGIC SELECT analyze_clinical_notes('''
# MAGIC   CONCLUSÃO: A administração de ativador tissular do plasminogênio foi responsável pela grande extensão da hemorragia e deve ser considerada no diagnóstico diferencial de descolamento hemorrágico coroideano.
# MAGIC ''') AS summary
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TABLE summaries AS
# MAGIC SELECT id, text, is_ADE, s.* FROM (
# MAGIC   SELECT *, analyze_clinical_notes(text) AS s FROM clinical_notes_pt)
...
# MAGIC %sql
# MAGIC SELECT is_ADE, presenca_evento, count(*) FROM summaries WHERE presenca_evento IS NOT NULL GROUP BY ALL
...
# MAGIC %sql
# MAGIC SELECT SUM(CASE WHEN (is_ADE = TRUE AND presenca_evento = 'S') OR (is_ADE = FALSE AND presenca_evento = 'N') THEN 1 END) / COUNT(*) AS acuracidade FROM summaries
...
# MAGIC %sql
# MAGIC SELECT droga, count(*) AS cnt FROM (SELECT explode(drogas) AS droga FROM summaries)
# MAGIC GROUP BY droga ORDER BY cnt DESC LIMIT 10
...
# MAGIC %sql
# MAGIC SELECT evento, count(*) AS cnt FROM (SELECT explode(eventos) AS evento FROM summaries)
# MAGIC GROUP BY evento ORDER BY cnt DESC LIMIT 10
...
# MAGIC %sql
# MAGIC SELECT droga, evento, count(*) AS cnt FROM (SELECT explode(drogas) AS droga, explode(eventos) AS evento FROM vr_demo.hls_ade.summaries)
# MAGIC GROUP BY ALL ORDER BY cnt DESC LIMIT 100
...
# MAGIC %sql
# MAGIC WITH 
# MAGIC   e AS (SELECT evento, count(*) AS cnt FROM (SELECT explode(eventos) AS evento FROM summaries) GROUP BY evento HAVING cnt > 2),
# MAGIC   a AS (SELECT droga, evento, count(*) AS cnt FROM (SELECT explode(drogas) AS droga, explode(eventos) AS evento FROM vr_demo.hls_ade.summaries) GROUP BY ALL)
# MAGIC SELECT a.droga, a.evento, a.cnt/e.cnt AS pct FROM a INNER JOIN e ON a.evento = e.evento ORDER BY pct DESC LIMIT 100
