...
model = ChatDatabricks(endpoint="databricks-dbrx-instruct", max_tokens = 800)

def translate_to_pt(text):

  prompt = [
    SystemMessage(content="Traduza para Português o texto a seguir (não adicione nenhum texto além da tradução):"),
...
translate_to_pt("CONCLUSION: The administration of tissue plasminogen activator was responsible for the large extent of hemorrhage and should be considered in the differential diagnosis of hemorrhagic choroidal detachment.")
...
(spark
  .read.table('clinical_notes').repartition(16)
  .select('id', translate_udf(col('text')).alias('text'), 'is_ADE')
  .write.mode('overwrite').saveAsTable('clinical_notes_pt2')
)
