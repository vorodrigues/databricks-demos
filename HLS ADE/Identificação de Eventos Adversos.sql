...
select is_ADE, count(*) as from clinical_notes_pt group by is_ADE
...
-- MAGIC
-- MAGIC Para simplificar a consulta aos modelos de Gen AI, vamos utilizar a função nativa **`AI_QUERY`**. Esta permite usar os modelos fundacionais através de simples consultas SQL. No nosso caso, usaremos o **`LLAMA 3`**.
-- MAGIC
-- MAGIC Agora, vamos usar esses recursos para identificar as seguintes informações:
-- MAGIC * Presença de evento adverso
-- MAGIC * Tipo do evento adverso
-- MAGIC * Drogas
...
-- MAGIC
-- MAGIC Primeiro, iremos criar uma função para facilitar a extração das informações
...
-- MAGIC %md Com isso, podemos testar a extração
...
-- MAGIC
-- MAGIC Por fim, vamos aplicar nosso modelo de forma massiva em todo o nosso conjunto de dados
