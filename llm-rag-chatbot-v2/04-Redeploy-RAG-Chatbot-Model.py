...
mosaic_route_name = "mosaicml-llama2-70b-chat-VR"
...
class ChatbotRAG(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input):
        ...
        for question in model_input:
          ...
          #Find relevant documents in Vector Search
...
results = proxy_model.predict(None, ["What is the best song of all time?"])
print(results[0]["answer"])
...
with mlflow.start_run(run_name="chatbot_rag-VR") as run:
...
serving_client.create_endpoint_if_not_exists("chatbot_rag-VR", 
                                            model_name=f"{catalog}.{db}.chatbot-VR", 
...
answer = requests.post(f"{serving_client.base_url}/serving-endpoints/chatbot_rag-VR/invocations", 
                       json={"dataframe_split": {"data": [question]}}, 
                       headers=serving_client.headers).json()
