...
# MAGIC %pip install mlflow==2.10.1 langchain==0.1.8 databricks-vectorsearch==0.22 databricks-sdk==0.18.0 mlflow[databricks]
...
test_demo_permissions(host, secret_scope=token_scope, secret_key=token_secret, vs_endpoint_name=VECTOR_SEARCH_ENDPOINT_NAME, index_name=index_name, embedding_endpoint_name="databricks-bge-large-en")
...
host = "https://" + spark.conf.get("spark.databricks.workspaceUrl")
os.environ['DATABRICKS_TOKEN'] = dbutils.secrets.get(token_scope, token_secret)
...
print(f"Documentos relevantes: {similar_documents[0]}")
...
TEMPLATE = """Você é um assistente para usuários do Databricks. Siga as intruções abaixo:
- Não responda perguntas que não sejam sobre Databricks, Apache Spark, Python, SQL, engenharia de dados, ciência de dados, DW, API ou administração de infraestrutura relacionada ao Databricks.
...
question = {"query": "Como posso acompanhar meus custos com Databricks?"}
...
endpoint_config = EndpointCoreConfigInput(
    ...
    served_models=[
        ServedModelInput(
            ...
            environment_vars={
                "DATABRICKS_TOKEN": "{{secrets/"+token_scope+"/"+token_secret+"}}",  # <scope>/<secret> that contains an access token
            }
        )
    ]
)
